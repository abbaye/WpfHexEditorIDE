...
| **📐 `.whfmt` Format Engine** | Proprietary unified definition language — **single source of truth** for both binary formats and programming languages. **460+ definitions** ship built-in. **For binary formats**: `repeating` blocks, `union` types, `versionedBlocks` (PE32/PE64 branching), `pointer` resolution, `checksum` validation (CRC32, Adler32, MD5, SHA…), `assertion` rules with forensic alerts, `navigation` bookmarks, `aiHints` for AI analysis — drives Parsed Fields tree, hex overlay coloring, format detection, and Structure Editor. **For source languages** (55+ definitions): syntax highlighting rules (keywords, strings, comments, operators, regex patterns), folding strategies (brace/indent/region/tag), bracket pair definitions, comment toggle patterns (`//` / `/* */`), auto-close pairs, indentation rules, end-of-block hint settings, breakpoint validation patterns (non-executable line detection), and whitespace rendering mode — the Code Editor reads **all** its language behavior from `.whfmt` files, zero hardcoded language logic |
