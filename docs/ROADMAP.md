...
>
> **Current IDE build:** `0.6.5.95` — ScreenRecorder plugin, ClassDiagram live arrows, Scripting Console, Snippets Phase 2, ByteProvider v1.3.0, Hex Diff/Patch, Binary Analysis, Welcome Screen Redesign.
...
| #156 | **Class Diagram Plugin** | 🔧 **~75% Done** — DSL v2 + syntax-highlighted pane (`classdiagram.whfmt` + `CodeEditorSplitHost`), 3 layout strategies (Force-Directed / Hierarchical / Swimlane), canvas minimap drag + corner-snap + hide, left-panel TreeView with colored selectable members, collapsible sections + dual metrics badge, hover tooltips (400 ms), context menu (ZoomToRect, clipboard export), scrollbars, session state save/restore; live arrow redraw during drag ✅ v0.6.5.95; round-trip Roslyn sync phases 1A–7 done (ADR-021–036); SVG exporter (`ClassDiagramSvgExporter`) ✅. Remaining: incremental Roslyn analysis, PNG export. | ~75% |
...
| #171 | **DiffViewer Phase 2** | ✅ **Done** — `DiffViewerFactory`; `DiffViewerViewModel`; side-by-side + unified diff view; chunk-level accept/reject. Remaining: 3-way merge, `.patch` export, word-level diff. |
