...
namespace WpfHexaEditor.Views.Dialogs
{
    /// <summary>
            ...
            foreach (var diff in _differences)
            {
                ...
                switch (diff.Type)
                {
                    case DifferenceType.Modified:
                        _editor1.AddCustomBackgroundBlock(new Core.CustomBackgroundBlock
                        {
                            StartOffset = diff.Offset,
                            Length = diff.Length,
                            Color = new System.Windows.Media.SolidColorBrush(color)
                        });
                        _editor2.AddCustomBackgroundBlock(new Core.CustomBackgroundBlock
                        {
                            StartOffset = diff.Offset,
                            Length = diff.Length,
                            Color = new System.Windows.Media.SolidColorBrush(color)
