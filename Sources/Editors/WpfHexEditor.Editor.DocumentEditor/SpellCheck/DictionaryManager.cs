...
// File: SpellCheck/DictionaryManager.cs
// Description:
//     Manages Hunspell dictionary files (.dic/.aff) stored in
//     %APPDATA%\WpfHexEditor\Dictionaries\{lang}\main.dic|aff.
//     Supports install from file (Browse) and from URL (Download).
// Architecture:
//     Stateless — all state is derived from the file system.
//     InstallFromUrlAsync downloads from LibreOffice dictionary mirror.
// ==========================================================
...
{
    // Supported languages: code → (display name, LibreOffice repo sub-path, dic prefix)
    ...
    public IReadOnlyList<DictionaryInfo> GetAllLanguages()
    {
        ...
        foreach (var (code, (display, _, prefix)) in KnownLanguages)
        {
            ...
            result.Add(new DictionaryInfo(code, display,
                File.Exists(dic) && File.Exists(aff), dic, aff));
        }
        return result;
    }
    ...
    public DictionaryInfo? GetInfo(string languageCode)
    {
        ...
        return new DictionaryInfo(languageCode, meta.Display,
            File.Exists(dic) && File.Exists(aff), dic, aff);
    }
    ...
    /// <summary>Installs a dictionary from user-supplied .dic and .aff files.</summary>
    ...
    /// <summary>
    /// Downloads a dictionary from the LibreOffice mirror.
    /// Reports 0..1 progress via <paramref name="progress"/>; -1 = indeterminate.
    /// </summary>
        ...
        CancellationToken      ct          = default)
    {
        ...
        progress?.Report(-1); // indeterminate while connecting

        var base_ = _settings.MirrorUrl.TrimEnd('/');
        await DownloadFileAsync($"{base_}/{meta.RepoPath}/{meta.Prefix}.dic", dic, progress, 0.0, 0.5, ct);
        await DownloadFileAsync($"{base_}/{meta.RepoPath}/{meta.Prefix}.aff", aff, progress, 0.5, 1.0, ct);
    ...
    public void Remove(string languageCode)
    {
        ...
        try { Directory.Delete(Path.GetDirectoryName(dic)!); } catch { /* not empty */ }
    }
